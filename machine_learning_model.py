...
class MachineLearningModel(object):
    ...
    def transform_for_cross_data( self, date_for_slice = "2019Q4" ):
        ...
        columns_to_use_list = columns_to_use_list + \
                              utils.do_find_and_select_from_list(self.input_cross.columns, search_list_balance,
                                                                 exclusion_list_balance)
    ...
    def import_data_to_model(self, import_command, last_date = "", first_date = ""):
        ...
        if import_command == "cross_df":
            self.transform_to_different_sets("cross_df", first_date = first_date, last_date = last_date)
            self.imported_data = self.cross_df.copy()

        if import_command == "cross_long_df":
            self.transform_to_different_sets("cross_long_df", first_date = first_date, last_date = last_date)
            self.imported_data = self.cross_long_df.copy()

        elif import_command == "panel_df":
            self.transform_to_different_sets("panel_df", first_date = first_date, last_date = last_date)
            self.imported_data = self.panel_df.copy()
